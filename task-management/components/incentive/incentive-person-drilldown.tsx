  ...
  return (
    <Dialog.Root open={open} onOpenChange={(o) => !o && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[100] -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl rounded-section bg-surface-card border border-hairline p-7 max-md:p-5 shadow-lg max-h-[calc(100dvh-32px)] overflow-y-auto">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <Dialog.Close asChild>
              <button
                type="button"
                aria-label="Close"
                className="inline-flex items-center justify-center h-9 w-9 rounded-full text-ink-subtle hover:bg-surface-soft transition-colors shrink-0"
              >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2.5 max-sm:grid-cols-1">
        ...
        <StatChip label="Unpaid" value={formatInr(totals.totalUnpaid)} tone="red" />
      </div>
...
function StatChip({ label, value, tone }: { label: string; value: string; tone: "ink" | "green" | "red" }) {
  const color =
    tone === "green" ? "var(--color-green-deep)" : tone === "red" ? "var(--color-red-deep)" : "var(--color-ink-strong)";
  return (
    <div className="rounded-chip border border-hairline bg-surface-soft px-4 py-3">
      <div className="uppercase font-black tracking-[0.06em] text-ink-subtle" style={{ fontSize: 10.5 }}>
        {label}
      </div>
      <div className="tabular-nums font-black mt-1" style={{ fontSize: 19, color }}>
        {value}
      </div>
    </div>
  );
...
function SectionHead({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex items-center gap-2 mb-2.5 text-ink-strong">
      <span className="inline-flex items-center justify-center h-6 w-6 rounded-lg" style={{ background: "var(--color-surface-soft)" }}>
        {icon}
      </span>
